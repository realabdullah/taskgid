		...
		<SidebarMenuItem>
			<DropdownMenu>
				<DropdownMenuTrigger as-child>
					<SidebarMenuButton size="lg" class="hover:bg-sidebar-accent data-[state=open]:bg-sidebar-accent h-10 rounded-md px-2">
						<Avatar class="h-8 w-8 shrink-0">
							<AvatarImage :src="user?.profilePicture || ''" :alt="user?.username" />
							<AvatarFallback class="bg-sidebar-accent text-sidebar-foreground">{{ getInitials(user?.firstName, user?.lastName) }}</AvatarFallback>
						</Avatar>
						<div class="grid flex-1 text-left leading-tight group-data-[collapsible=icon]:hidden">
							<span class="text-sidebar-foreground truncate text-sm font-medium">{{ user?.firstName }} {{ user?.lastName }}</span>
							<span class="text-2xs text-sidebar-foreground/50 truncate font-mono">Member</span>
						</div>
