...
<template>
	<div class="flex items-center gap-2">
		<Popover>
			<PopoverTrigger as-child>
				<Button variant="outline" size="sm" class="h-8 gap-1">
					...
					<Badge v-if="activeFilterCount > 0" variant="secondary" class="ms-1 rounded-full px-1 py-0 text-xs tabular-nums"> {{ activeFilterCount }} </Badge>
				</Button>
			</PopoverTrigger>

			<PopoverContent class="border-border bg-surface-0 w-[calc(100vw-1rem)] max-w-[340px] p-0" align="start">
				<div class="border-border shrink-0 space-y-1 border-b p-4">
					<h4 class="text-text-primary text-sm leading-none font-semibold">Filter team members</h4>
					<p class="text-text-tertiary text-xs tabular-nums">Showing {{ totalMembers || 0 }} team members</p>
				</div>
				...
				<div class="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
					...
					<div class="space-y-4">
						<div class="grid grid-cols-2 gap-2">
							<div class="space-y-2">
								<Label html-for="minTasksAssigned">Minimum tasks assigned</Label>
								<Input id="minTasksAssigned" v-model="filter.minTasksAssigned" type="number" placeholder="Min" />
							</div>
							<div class="space-y-2">
								<Label html-for="maxTasksAssigned">Maximum tasks assigned</Label>
						...
						<div class="grid grid-cols-2 gap-2">
							<div class="space-y-2">
								<Label html-for="minTasksCompleted">Minimum tasks completed</Label>
								<Input id="minTasksCompleted" v-model="filter.minTasksCompleted" type="number" placeholder="Min" />
							</div>
							<div class="space-y-2">
								<Label html-for="maxTasksCompleted">Maximum tasks completed</Label>
								...
								<SelectContent>
									<SelectItem value="ASC">
										<div class="flex items-center">
											<Icon name="hugeicons:sort-by-up-02" :size="16" class="me-2" />
											Ascending
										</div>
									</SelectItem>
									<SelectItem value="DESC">
										<div class="flex items-center">
											<Icon name="hugeicons:sort-by-down-02" :size="16" class="me-2" />
											Descending
										</div>
									</SelectItem>
								</SelectContent>
				...
				<div class="border-border bg-surface-1 flex shrink-0 items-center justify-between gap-2 border-t px-4 py-3">
		...
		<Button v-if="activeFilterCount > 0" variant="ghost" size="sm" class="text-text-secondary h-8 px-2" @click="resetFilters">
			<Icon name="hugeicons:cancel-01" :size="14" class="me-1" />
			Clear filters
		</Button>
	</div>
</template>
