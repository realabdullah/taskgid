...
import { usePasskeys } from "~/features/identity/composables/usePasskeys";
	...
	<div class="space-y-4 pb-4">
		...
		<div class="space-y-4">
			<div class="space-y-0.5">
				...
				<p class="text-muted-foreground text-sm">Passkeys let you sign in with your fingerprint, face, or device PIN.</p>
			</div>
			...
			<div class="mt-4 space-y-3">
				...
				<div v-else class="bg-muted rounded-md p-4 text-center">
					<p class="text-muted-foreground">No passkeys yet</p>
				</div>
			</div>

			<Button :loading="addingPasskey" loading-label="Adding passkey" class="min-w-[170px]" :disabled="addingPasskey" @click="handleAddPasskey">
				<template v-if="addingPasskey">Adding passkey…</template>
				<template v-else>
