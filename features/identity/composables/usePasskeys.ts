...
export const usePasskeys = () => {
	...
	const handleAddPasskey = async () => {
		try {
			...
			if (!success || !options || error) throw new Error(error || "Unable to start passkey setup. Try again.");

			const attResp = await withChallengeTimeout(startRegistration({ optionsJSON: options as any }), "Passkey setup timed out after 60 seconds. Try again.");
			if (!attResp) throw new Error("Unable to create the passkey. Try again.");
			...
			if (!res.success || res.error) throw new Error(res.error || "Unable to save the passkey. Try again.");
	...
	const handleRemovePasskey = async (passkey: Passkey) => {
		...
		try {
			...
			if (!res?.success || res.error) {
				throw new Error(res?.error || "Unable to remove the passkey. Try again.");
			}
	...
	const loginWithPasskey = async (email: string) => {
		...
		if (!success || !options || error) throw new Error(error || "Unable to start passkey sign-in. Try again or use your password.");

		const attResp = await withChallengeTimeout(startAuthentication({ optionsJSON: options as any }), "Passkey sign-in timed out after 60 seconds. Try again.");
		if (!attResp) throw new Error("Unable to verify the passkey. Try again or use your password.");
