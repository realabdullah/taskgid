...
const authnLogin = async () => {
	try {
		if (form.email.trim() === "") return push.error("Please fill all the required fields.");
		submitting.value = true;
		const { success, accessToken, refreshToken } = await loginWithAuthn(form.email);
		if (!success) throw new Error("We couldn't log you in. Please try again.");
		setToken(accessToken, refreshToken);
		submitting.value = false;
		push.success("Logged in successfully.");
		...
		submitting.value = false;
		push.error("Invalid email or passkey provided.");
	}
};
</script>
...
<template>
	<NuxtLayout name="auth">
		...
		<div class="login__form flex flex-column content-center items-start w-100 h-100">
			...
			<form class="flex flex-column content-center items-center w-100" @submit.prevent="usePasskey ? authnLogin() : submitForm()">
				<BaseInput id="email" v-model="form.email" type="email" label="Email Address" :errors="v$.email.$errors" />
				<BaseInput v-if="!usePasskey" id="password" v-model="form.password" type="password" label="Enter Your Password" :errors="v$.password.$errors" />
